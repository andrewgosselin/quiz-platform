<x-app-layout>
    ...
    <x-slot name="scripts">
        ...
        <script>
            ...
            function deleteQuestion(id) {
                $.ajax({
                    type:'DELETE',
                    url:"{{ route('ajax.question.delete', ['id' => $quiz->id ?? '-1']) }}/" + id,
                    success:function(data){
            ...
            function saveQuestion() {
                ...
                $.ajax({
                    type:'POST',
                    url: "{{ route('ajax.question.update', ['id' => $quiz->id ?? '-1']) }}/" + id,
                    data: getQuestionData(),
                    contentType: false,
                    processData: false,
                    success:function(data){
            ...
            function submitQuiz() {
                ...
                data.append('prize', $("#prizeInput").val());
                data.append('image_file', $('#quizImageUploadInput')[0].files[0]); 
                data.append('category_id', $('#categorySelect').val());
                ...
                $.ajax({
                    type:'POST',
                    url:"{{ route('ajax.quiz.update', ['id' => $quiz->id ?? 'new']) }}",
                    data: data,
                    contentType: false,
                    processData: false,
                    success:function(data){
    ...
    <div class="pt-4">
        ...
        <div class="mb-3">
            ...
            <select id="categorySelect" class="form-select" aria-label="Default select example">
                @foreach($categories as $category)
                    @if($isEditing && $quiz->category_id == $category->id)
        ...
        <div class="mb-3">
            <label for="prizeInput" class="form-label">Prize</label>
            <input type="number" class="form-control" id="prizeInput" value="{{$quiz->prize ?? 0}}">
        </div>
